    ...
    const handleAddOperator = async (e) => {
      ...
      try {
        ...
        if (process.env.NODE_ENV !== 'production') {
          const formDataToSend = new FormData();
          formDataToSend.append('file', formData.file);
          const uploadRes = await axios.post('http://localhost:5001/upload', formDataToSend, {
            headers: { 'Content-Type': 'multipart/form-data' },
          });
          finalImagePath = uploadRes.data.imagePath;
        }
        ...
        const res = await axios.post(
          `https://op-backend-d1b9.onrender.com/api/operators/${line}`,
          operatorData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
        ...
        <div className="bg-white p-6 rounded shadow-lg w-3/4 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
          ...
          {showAddForm && (
            <div className="mt-4">
              ...
              <button onClick={handleAddOperator} className="bg-blue-500 text-white px-4 py-2 rounded">
                Add Operator
              </button>
            </div>
          )}
